...
| **Config theft** | Someone copies your `.claude/` directory | Challenge phrase is stored as salted hash in secure storage. Canary traps detect them. |
...
| **TOTP replay** | Reuse authenticator code in same time-step | Accepted TOTP counter is tracked (`totp_last_counter`) and replay is rejected |
...
| **Side-channel attacks** | Local timing attacks remain theoretically possible in shell scripts, but ai2fa avoids early-return comparisons for secrets. |
...
6. **Pick a security level that matches your tolerance** — `low` for daily use, `medium/high/extra_high` for higher-risk contexts
